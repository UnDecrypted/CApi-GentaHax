...
if pcall(function()player():px()end) then
  ...
  function player()
    ...
    function tab:pathfind(x,y)
      function astar(x,y,srad)
        function step()
          local dump = {}
          for mx=0, srad-1 do
            for my=0, srad-1 do
              if checkPath(math.floor(getLocal().pos.x/32)-math.floor(srad/2)+mx,math.floor(getLocal().pos.y/32)-math.floor(srad/2)+my) then
                local jarax = math.abs((math.floor(getLocal().pos.x/32)-math.floor(srad/2)+mx)-x)
                local jaray = math.abs((math.floor(getLocal().pos.y/32)-math.floor(srad/2)+my)-y)
                table.insert(dump,{pos={x=math.floor(getLocal().pos.x/32)-math.floor(srad/2)+mx,y=math.floor(getLocal().pos.y/32)-math.floor(srad/2)+my},jarax=jarax,jaray=jaray})
              end
            end
          end
          local closest = {pos={x=0,y=0},jarax=1000,jaray=1000}
          for i, v in pairs(dump) do
            doLog("List : "..v.jarax..","..v.jaray)
            if v.jarax < closest.jarax and v.jaray < closest.jaray then
              closest = v
            end
          end
          doLog("Closest : "..closest.jarax..","..closest.jaray)
          return closest
        end
        while true do
          local best = step()
          findPath(best.pos.x,best.pos.y)
          sleep(100)
          if math.floor(getLocal().pos.x/32) == x and math.floor(getLocal().pos.y/32) == y then
            break
          end
        end
      end
  ...
  function players()
    ...
    function tab:getbyname(name)
      for i, v in pairs(getPlayerlist()) do
        if v.name:find(name) then
          local ptab = {player = v}
          function ptab:pull()
            local rawname = ptab.player.name:match("`%d(.+)``")
            sendPacket(2,"action|input\n|text|/pull "..rawname)
          end
          function ptab:kick(ptab)
            local rawname = ptab.player.name:match("`%d(.+)``")
            sendPacket(2,"action|input\n|text|/kick "..rawname)
          end
          function ptab:ban(ptab)
            local rawname = ptab.player.name:match("`%d(.+)``")
            sendPacket(2,"action|input\n|text|/ban "..rawname)
          end
          return ptab
  ...
  function tile()
    ...
    function tab:vend(x,y)
      local tab = {}
      local vid = getExtraTile(x,y).lastUpdate
      local vprice = getExtraTile(x,y).owner
      function tab:buy(amount)
        sendPacket(2,"action|dialog_return\ndialog_name|vending\ntilex|"..x.."|\ntiley|"..y.."|\nverify|1|\nbuycount|"..amount.."|\nexpectprice|"..vprice.."|\nexpectitem|"..vid.."|\n")
      end
      function tab:edit()
        local tab = {}
        ...
        function tab:price(price,pritem,prlock)
          sendPacket(2,"action|dialog_return\ndialog_name|vending\ntilex|59|\ntiley|111|\nsetprice|"..price.."\nchk_peritem|"..booltoint(pritem).."\nchk_perlock|"..booltoint(prlock).."\n")
        end
        return tab
      end
      return tab
    ...
    function tab:sucker(x,y)
      local tab = {}
      function tab:retrive(amount)
        sendPacket(2,"action|dialog_return\ndialog_name|itemremovedfromsucker\ntilex|"..x.."|\ntiley|"..y.."|\nitemtoremove|"..amount.."\n")
      end
      function tab:add(amount)
        sendPacket(2,"action|dialog_return\ndialog_name|itemaddedtosucker\ntilex|"..x.."|\ntiley|"..y.."|\nitemtoadd|"..amount.."\n")
      end
      function tab:clear()
        sendPacket(2,"action|dialog_return\ndialog_name|itemsucker_block\ntilex|"..x.."|\ntiley|"..y.."|\nbuttonClicked|clearitem\n\nchk_enablesucking|1\n")
      end
      function tab:select(id)
        sendPacket(2,"action|dialog_return\ndialog_name|itemsucker_block\ntilex|"..x.."|\ntiley|"..y.."|\nselectitem|"..id.."\n")
      end
      return tab
    end
  ...
  if pcall(function()RemoveHook("var")end) then
...
add_label_with_icon|big|`cCApi News``|left|3524|
add_textbox|`c[==================================================================]|left|
add_label_with_icon|small|`1Hello, ]]..getLocal().name..[[``|left|9474|
add_label_with_icon|small|`1Today : [ ]]..os.date("%d %B, %Y")..[[ ]``|left|9474|
add_label_with_icon|small|`1Time : [ ]]..os.date("%H : %M")..[[ ]``|left|9474|
add_textbox|`c[==================================================================]|left|
NEWSDATA
end_dialog|zz|Close||
add_quick_exit|
          ...
          for i, v in pairs(data) do
            dump = dump.."add_label_with_icon|small|"..v.." ]``|left|9474|\n"
          end
          NewsDialog = NewsDialog:gsub("NEWSDATA",dump)
          var = {}
          var[0] = "OnDialogRequest"
          var[1] = NewsDialog
          sendVariant(var)
