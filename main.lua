...
if
 ...
 then
    ...
    function player()
        ...
        function tab:tp(x, y)
            sendVariant({[0] = "OnSetPos", [1] = {x = x * 32, y = y * 32}}, getLocal().netId)
        end
                ...
                function step()
                    ...
                    for mx = 0, srad - 1 do
                        for my = 0, srad - 1 do
                            if
                                checkPath(
                                    math.floor(getLocal().pos.x / 32) - math.floor(srad / 2) + mx,
                                    math.floor(getLocal().pos.y / 32) - math.floor(srad / 2) + my
                                )
                             then
                                local jarax =
                                    math.abs((math.floor(getLocal().pos.x / 32) - math.floor(srad / 2) + mx) - x)
                                local jaray =
                                    math.abs((math.floor(getLocal().pos.y / 32) - math.floor(srad / 2) + my) - y)
                                table.insert(
                                    dump,
                                    {
                                        pos = {
                                            x = math.floor(getLocal().pos.x / 32) - math.floor(srad / 2) + mx,
                                            y = math.floor(getLocal().pos.y / 32) - math.floor(srad / 2) + my
                                        },
                                        jarax = jarax,
                                        jaray = jaray
                                    }
    ...
    AddHook("OnVarlist", "var", diahook)
    function txthook(type, pkt)
        if type == 2 then
            local cmd = pkt:match("text|/(%w+)")
            if cmd then
                cmd = cmd:upper()
                if cmd == "NEWS" then
                    var = {}
    ...
    AddHook("OnTextPacket", "txt", txthook)
    ...
    function announcement()
        while true do
            ...
            if file then
                ...
                local mes =
                    makeRequest(
                    "https://raw.githubusercontent.com/UnDecrypted/CApi-GentaHax/main/announcement.txt",
                    "GET"
                ).content
                if tostring(clientmes) == tostring(mes) then
                    ...
                    var = {}
                    var[0] = "OnTextOverlay"
                    var[1] = "`b[`c/news`b] `cTo Open CApi News"
                    sendVariant(var)
                end
